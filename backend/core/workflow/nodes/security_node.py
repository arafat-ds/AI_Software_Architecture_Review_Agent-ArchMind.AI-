"""LangGraph node: SecurityAnalysisNode.

Responsibilities:
- Validate that parsed_code_representation is present in state
- Call SecurityService to run rule engine + Gemini generation
- Write security_section to AnalysisState
...
from services.security_agent import SecurityService
...
_NODE_NAME = "SecurityAnalysisNode"
_service: SecurityService | None = None


def _get_service() -> SecurityService:
    """Return the SecurityService singleton, initialising it on first call."""
    global _service
    if _service is None:
...
def security_analysis_node(state: AnalysisState) -> dict[str, Any]:
    """Run the Security Analysis Agent and write SecuritySection to state.
    ...
    Raises:
        FatalNodeError: Always on security analysis failure.
    """
    ...
    execution = begin_node_execution(
        state, _NODE_NAME, output_field="security_section"
    )

    logger.info("SecurityAnalysisNode started", extra={
    ...
    try:
        ...
        logger.info("SecurityAnalysisNode complete", extra={
            "job_id": str(state["job_id"]),
        ...
        logger.error("SecurityAnalysisNode failed", extra={
            "job_id": str(state["job_id"]),
